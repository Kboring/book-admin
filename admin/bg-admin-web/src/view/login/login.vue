    ...
    <Layout>
      ...
      <Content :style="{ background: '#fff', minHeight: '500px'}">
        <div style="float: right;margin: 100px 100px 0">
          <Card title="欢迎登录boringKey-图书管理系统">
        ...
        this.$refs['loginForm'].validate((valid) => {
          if (valid) {
            login(this.loginForm).then(res=>{
              if(res.code==200){
                //localStorage.setItem("loginName",this.loginForm.loginAccount)
                sessionStorage.setItem("login_name",res.obj.login_name)
                sessionStorage.setItem("name",res.obj.name)
                sessionStorage.setItem("password",res.obj.password)
                sessionStorage.setItem("telephone",res.obj.telephone)
                sessionStorage.setItem("email",res.obj.email)
                sessionStorage.setItem("department_name",res.obj.department_name)
                this.$router.push({
                  name: 'main'
                })
              }else{
